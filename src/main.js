...
async function openSaleDetail(id){
  try{
    ...
    const itemHtml=s.items.map(i=>{
      ...
      const btn=canReturn?`<button class="btn ghost returnBtn" data-id="${i.id}" style="float:right">Reso</button>`:'';
      return `<div class="card" style="margin-bottom:8px"><strong>${i.brand} ${i.name}</strong><br><small class="muted">Barcode: ${i.barcode||i.internal_code||'—'}</small><br><span>${i.quantity} × ${euro(i.final_unit_price)}</span>${btn}</div>`;
    }).join('');
    const cancelBtn=state.admin && s.status==='completed'?'<button class="btn danger" id="cancelSale" style="width:100%;margin-top:10px">Annulla vendita</button>':'';
    const m=modal({title:`Vendita ${s.sale_code}`,body:`<p><strong>${euro(s.total)}</strong> · ${new Date(s.occurred_at).toLocaleString('it-IT')}</p>${itemHtml}${cancelBtn}`});
    m.querySelectorAll('.returnBtn').forEach(b=>b.onclick=async()=>{const reason=prompt('Motivo del reso:','');if(reason===null)return;await api('/api/returns',{method:'POST',body:JSON.stringify({sale_item_id:+b.dataset.id,quantity:1,reason})});m.remove();refresh()});
