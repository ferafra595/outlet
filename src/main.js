...
function saleDisplayStatus(s){if(s.status!=='completed')return {label:'Annullata',cls:'bad'};const returned=Number(s.returned_amount)||0;if(returned<=0)return {label:'Completata',cls:'ok'};if(returned+0.005>=Number(s.total||0))return {label:'Resa totalmente',cls:'warn'};return {label:'Reso parziale',cls:'warn'}}
function sales(){return `<div class="section-title"><h2>Vendite</h2><button class="btn" id="newSale">＋ Registra</button></div><div class="table-wrap"><table><thead><tr><th>Codice vendita</th><th>Barcode prodotti</th><th>Data</th><th>Articoli</th><th>Sconto</th><th>Vendita originale</th><th>Resi</th><th>Netto</th><th>Stato</th><th></th></tr></thead><tbody>${state.sales.map(s=>{const st=saleDisplayStatus(s);const returned=Number(s.returned_amount)||0;return `<tr><td>${s.sale_code}</td><td><small>${(s.product_barcodes||'—').split(',').join('<br>')}</small></td><td>${new Date(s.occurred_at).toLocaleString('it-IT')}</td><td>${s.items_count}</td><td>${euro(s.discount_total)}</td><td><strong>${euro(s.total)}</strong></td><td>${euro(returned)}</td><td><strong>${euro(Math.max(0,Number(s.total)-returned))}</strong></td><td><span class="badge ${st.cls}">${st.label}</span></td><td><button class="btn ghost saleDetail" data-id="${s.id}">Apri</button></td></tr>`}).join('')}</tbody></table></div>`}
...
async function openSaleDetail(id){
  try{
    ...
    const cancelBtn=state.admin && s.status==='completed' && returnedAmount===0?'<button class="btn danger" id="cancelSale" style="width:100%;margin-top:10px">Annulla vendita completa</button>':'';
